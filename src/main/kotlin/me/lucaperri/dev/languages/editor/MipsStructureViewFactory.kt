...
class MipsStructureViewFactory : PsiStructureViewFactory {
    override fun getStructureViewBuilder(psiFile: PsiFile): StructureViewBuilder? {
        if (psiFile !is MipsFile) return null
        return object : TreeBasedStructureViewBuilder() {
            override fun createStructureViewModel(editor: Editor?): StructureViewModel =
                MipsStructureViewModel(psiFile)
        }
    }
}

private val SECTION_DIRECTIVES = setOf(".text", ".data", ".bss", ".rodata", ".kdata", ".ktext")

private class MipsStructureViewModel(file: MipsFile) :
    StructureViewModelBase(file, MipsFileTreeElement(file)),
    ...
    override fun isAlwaysLeaf(element: StructureViewTreeElement): Boolean =
        element !is MipsFileTreeElement
}

private class MipsFileTreeElement(private val file: MipsFile) :
    ...
    override fun getChildren(): Array<StructureViewTreeElement> {
        ...
        for (child in file.children) {
            when (child) {
                is MipsDirectiveStmt -> {
                    val text = child.firstChild?.text?.lowercase()
                    if (text != null && text in SECTION_DIRECTIVES) {
                        items += MipsSectionTreeElement(child)
                    }
                }
                is MipsLabelDef -> items += MipsLabelTreeElement(child)
            }
        }
...
private class MipsLabelTreeElement(private val stmt: MipsLabelDef) :
    StructureViewTreeElement, SortableTreeElement {
...
private class MipsSectionTreeElement(private val stmt: MipsDirectiveStmt) :
    StructureViewTreeElement, SortableTreeElement {
